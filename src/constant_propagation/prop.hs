...
update item []
  = item : []
update item@(id', number) state
  | isNothing result = item : state
  | otherwise        = [(id'', number') | (id'', number') <- state, id' /= id''] ++ [(id',number)]
  where
    result = lookup id' state
...
execStatement (DoWhile block exp') state
  | (eval exp' result) == 1 = execStatement (DoWhile block exp') result
  | otherwise               = result
  where
    result = execBlock block state
...
execBlock (statement : statements) state
  = execBlock statements (execStatement statement state)
