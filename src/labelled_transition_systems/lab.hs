...
states :: LTS -> [State]
states transition
  = nub (states' transition)
  where
    states' :: LTS -> [Int]
    states' []
      = []
    states' (((state1, state2), _) : trans)
      = (nub [state1, state2]) ++ (states' trans)

transitions :: State -> LTS -> [Transition]
transitions state transition
  = [ trans | trans@((state1, state2), _) <- transition, state1 == state]

alphabet :: LTS -> Alphabet
alphabet transitiion
  = nub [str | ((_), str ) <- transitiion ]
...
actions :: Process -> [Id]
actions (Choice (process : processes))
  = nub (actions process) ++ (actions (Choice processes))
actions (Prefix str process)
  = nub ([str] ++ (actions process))
actions _
  = []
...
composeTransitions ((s, s'), id1) ((t ,t'), id2) a a' m
  | a == a' =
