...
checkSat (root, nodes) env
  | root == 0               = False
  | root == 1               = True
  | lookUp index env  = checkSat (rightNodeID, nodes) env
  | otherwise               = checkSat (leftNodeID, nodes) env
  where
    (index, leftNodeID, rightNodeID) = lookUp root nodes
...
sat (root, nodes)
  | root == 0     = []
  | root == 1     = [[]]
  | otherwise = satTrue ++ satFalse
  where
    (index, leftNode, rightNode) = lookUp root nodes
    satTrue = map ((index, True) :)  (sat (rightNode, nodes))
    satFalse = map ((index, False) :)  (sat (leftNode, nodes))
...
simplify (Or (Prim b1) (Prim b2))
  = Prim (b1 || b2)
simplify (Not (Prim b1))
  = Prim (not b1)
simplify x
  = x

restrict :: BExp -> Index -> Bool -> BExp
restrict  (IdRef num) index bool
  | num == index          = Prim bool
  | otherwise                 = IdRef num
restrict (Or exp1 exp2) index bool
  = simplify (Or (restrict exp1 index bool) (restrict exp2 index bool))
restrict (And exp1 exp2) index bool
  = simplify (And (restrict exp1 index bool) (restrict exp2 index bool))
restrict (Not exp1) index bool
  = simplify (Not (restrict exp1 index bool))
restrict x index bool
  = x
